...
def main():
    ...
    if len(sta_codes_unique) != len(all_stations):
        LOG.warning("WARNING_DUPLICATE_STATION: overall duplicate station "
                    "names, {} vs {} stations".format(len(sta_codes_unique),
                                                      len(all_stations)))
    ...
    with gzip.open("{}_{}".format(
            FLAGS.label, MISSING_ROUTES_GZIJPSON_FILE), 'wb') as fp:
...
def get_service_endpoint_per_channel(
        routing_service_endpoint, net, sta, cha,
        service='dataselect'):
...
def get_channels_from_single_station(root):
    ...
    for network in networks:
        ...
        if len(stations) > 1:
            LOG.warning("WARNING_STATION: "
                        "more than one Station elements in StationXML")
        ...
        for station in stations:
            ...
            for cha in channels:
                ...
                channel_data = {
                    ...
                    'sta_endtime': sta_et,
                    'longitude': float(cha.findtext(
                        STATIONXML_LONGITUDE_ELEMENT)),
                    'latitude': float(cha.findtext(
                        STATIONXML_LATITUDE_ELEMENT)),
                    'channel': cha,
